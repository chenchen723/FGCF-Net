...
class FrequencyRefineModule(nn.Module):
    ...
    def forward(self, x, high, freq):  # freq是64维，high192
        ...
        # freq = F.interpolate(freq, x.size()[2:], mode='bilinear', align_corners=True)
        high = self.conv1x1_fh(high)
        high = self.Double_ConvBnRule(high)
        # freq = self.conv1x1_fh(freq)
        # freq = self.Double_ConvBnRule(freq)
        ...
        ###################对低频进行通道注意力处理
        # freq_max = self.max_pool(freq)
        # freq_avg = self.avg_pool(freq)
        # freq_max_out = self.mlp(freq_max)
        # freq_avg_out = self.mlp(freq_avg)
        # freq_channel_out = self.sigmoid(freq_max_out + freq_avg_out)
        # freq_x = freq_channel_out * freq

        # freq_max_out, _ = torch.max(freq_x, dim=1, keepdim=True)  # 修正：使用freq_x而不是freq
        # freq_mean_out = torch.mean(freq_x, dim=1, keepdim=True)

        ####################原图与高频融合
        max_pool_x = self.max_pool(x)  # 重命名避免混淆
        avg_pool_x = self.avg_pool(x)
        
        high_max_combined = high_max + max_pool_x
        high_avg_combined = high_avg + avg_pool_x
        ...
        # 修正：直接相加，不要使用元组解包
        high_max_combined_out = high_max_out + max_out_high
        high_mean_combined_out = high_mean_out + avg_out_high
        ...
        # ####################原图与低频融合
        # freq_max_combined = freq_max + max_pool_x
        # freq_avg_combined = freq_avg + avg_pool_x
        # freq_max_out1 = self.mlp(freq_max_combined)
        # freq_avg_out1 = self.mlp(freq_avg_combined)
        # freq_channel_out1 = self.sigmoid(freq_max_out1 + freq_avg_out1)
        # freq_x_combined = freq_channel_out1 * x
        
        # freq1_max_out, _ = torch.max(freq_x_combined, dim=1, keepdim=True)
        # freq1_avg_out = torch.mean(freq_x_combined, dim=1, keepdim=True)

        # freq_max_combined_out = freq_max_out + freq1_max_out
        # freq_mean_combined_out = freq_mean_out + freq1_avg_out
        
        # freq_spatial_out = self.sigmoid(self.conv(torch.cat([freq_max_combined_out, freq_mean_combined_out], dim=1)))
        # freq_xx = freq_spatial_out * x
        
        out = high_xx
        out = self.conv_ll(out)
...
class AHFA(nn.Module):
    def __init__(self, in_channels1=64, in_channels2=64, in_channels3=128, out_channels=128, hamburger_config=None):
        ...
        self.conv1x1_2 = nn.Conv2d(in_channels2, in_channels2, kernel_size=1)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, fp5_or_di_1, fp5_i, boundary_feature):
        # Step 1: Transform fp5_or_di_1 to size of fp5_i
        if boundary_feature.shape[2:] != fp5_or_di_1.shape[2:]:
            scale_factor = boundary_feature.shape[2] // fp5_or_di_1.shape[2]
            if scale_factor > 1:
        ...
        x1 = torch.cat([fp5_or_di_1, boundary_feature], dim=1)
        x = torch.cat([boundary_feature, fp5_i], dim=1)
        x1 = self.conv1x1_up(x1)
        x = self.conv1x1_up(x)
...
class Double_ConvBnRule(nn.Module):
    ...
    def _init_pdc_kernels(self):
        ...
        # 固定PDC权重，不参与训练
        self.pdc_hv.weight.requires_grad = False
        self.pdc_dg.weight.requires_grad = False
        self.apdc.weight.requires_grad = False
